...
const FILE_EXTENSION = /\.(?:md|pdf|png|txt|json|csv|html?)/gi;
...
function validPathCandidate(path: string): boolean {
  if (
    ...
    || !/\.(?:md|pdf|png|txt|json|csv|html?)$/i.test(path)
  ) return false;
  const name = path.split("/").pop() || "";
  const extension = name.match(/\.(?:md|pdf|png|txt|json|csv|html?)$/i)?.[0] || "";
