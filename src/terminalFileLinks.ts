...
const FILE_EXTENSIONS = "md|pdf|png|svg|txt|json|csv|html?";
const FILE_EXTENSION = new RegExp(`\\.(?:${FILE_EXTENSIONS})`, "gi");
const FILE_EXTENSION_SUFFIX = new RegExp(`\\.(?:${FILE_EXTENSIONS})$`, "i");
...
function validPathCandidate(path: string): boolean {
  if (
    ...
    || !FILE_EXTENSION_SUFFIX.test(path)
  ) return false;
  const name = path.split("/").pop() || "";
  const extension = name.match(FILE_EXTENSION_SUFFIX)?.[0] || "";
